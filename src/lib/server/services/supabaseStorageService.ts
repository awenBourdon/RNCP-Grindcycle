...
export class SupabaseStorageService {
  ...
  private async ensureBucketExists(): Promise<void> {
    ...
    try {
      ...
      const bucketExists = buckets?.some(bucket => bucket.name === 'grindcycle-images')
  ...
  async deleteMultiple(urls: string[]): Promise<{ deleted: string[]; failed: string[] }> {
  ...
  private extractFilePathFromUrl(url: string): string | null {
    const match = url.match(/\/storage\/v1\/object\/public\/grindcycle-images\/(.+)$/)
