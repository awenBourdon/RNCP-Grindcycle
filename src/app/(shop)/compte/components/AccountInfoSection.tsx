...
export const AccountInfoSection = ({ session }: AccountInfoSectionProps) => {
  return (
    <div className="bg-white rounded-xl p-8 shadow-sm">
      <div className="flex items-center mb-8">
        <Settings size={24} className="text-[#0a3d3f] mr-3" />
        <h2 className="text-2xl font-normal text-[#010101]">Informations du compte</h2>
      </div>
      <div className="space-y-6">
        ...
        <div className="flex justify-between items-center py-4">
          ...
          <span className="text-[#010101] font-medium">{session.user.email}</span>
        </div>
      </div>
    </div>
  )
