...
export class EnhancedImageValidator {
  ...
  static async validateImage(file: File): Promise<EnhancedImageValidationResult> {
    ...
    try {
      ...
      if (dimensions) {
        ...
        if (dimensions.width > UPLOAD_CONFIG.imageDimensions.maxWidth || 
            dimensions.height > UPLOAD_CONFIG.imageDimensions.maxHeight) {
          result.errors.push(`Image trop grande (maximum ${UPLOAD_CONFIG.imageDimensions.maxWidth}x${UPLOAD_CONFIG.imageDimensions.maxHeight} pixels)`)
          result.isValid = false
        }
  ...
  private static validateBasicProperties(file: File): { isValid: boolean; errors: string[] } {
    const errors: string[] = []
    ...
    if (file.size > UPLOAD_CONFIG.maxFileSize) {
      errors.push(`Fichier trop volumineux (max ${UPLOAD_CONFIG.maxFileSize / (1024 * 1024)}MB)`)
    }
  ...
  private static validateMagicNumber(
    ...
    const expectedSignatures = this.MAGIC_NUMBERS[declaredMimeType as keyof typeof this.MAGIC_NUMBERS]
    
    if (!expectedSignatures) {
      errors.push(`Type MIME non supporté pour validation: ${declaredMimeType}`)
    ...
    if (!isValidSignature) {
      errors.push(`Le fichier n'est pas un vrai ${declaredMimeType} (signature invalide)`)
    }
  ...
  private static extractPngDimensions(bytes: Uint8Array): { width: number; height: number } | null {
  ...
  private static extractJpegDimensions(bytes: Uint8Array): { width: number; height: number } | null {
    let i = 2
  ...
  private static extractWebpDimensions(bytes: Uint8Array): { width: number; height: number } | null {
  ...
  static async validateMultipleImages(files: File[]): Promise<{
    ...
    if (files.length > UPLOAD_CONFIG.maxFiles.products) {
      globalErrors.push(`Trop de fichiers (max ${UPLOAD_CONFIG.maxFiles.products})`)
    }

    const totalSize = files.reduce((sum, file) => sum + file.size, 0)
    if (totalSize > UPLOAD_CONFIG.maxTotalSize) {
      globalErrors.push(`Taille totale trop importante (max ${UPLOAD_CONFIG.maxTotalSize / (1024 * 1024)}MB)`)
    }
    ...
    const allValid = globalErrors.length === 0 && results.every(r => r.validation.isValid)
